...
function InputCard({ set_Image, set_Temp, set_City, set_Country, set_Description, set_Wind_Speed, set_Humidity, set_Tempmax, set_Tempmin, set_Pressure, set_Pre }) {
    ...
    const CheckWeather = () => {
        ...
        axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${City},${Country}&APPID=0193b5d33ca87ec43b479e3ac7667f48&units=metric&units=metric`)
        .then((response) => {
            ...
            set_Tempmax(result.main.temp_max)
            set_Tempmin(result.main.temp_min)
            set_Pressure(result.main.pressure)
            set_Pre(result.clouds.all)
            set_City(City)
            set_Country(Country)
